...
const indexAsset = /(?:styles\.css|data\/prompts\.js|data\/catalog\.js|app\.js)\?v=(\d+)/g;
...
if (indexVersions.length !== 4 || appVersions.length !== 2) {
...
for (const file of files) {
  ...
  const output = file === 'index.html'
    ? sources[file].replace(
      new RegExp(`((?:styles\\.css|data/prompts\\.js|data/catalog\\.js|app\\.js)\\?v=)${previous}`, 'g'),
      `$1${next}`,
    )
